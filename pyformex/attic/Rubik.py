...
def createCube(n=3):
    tol = 0.005
    front = Formex('4:0123').replicm((n, n)).translate([-n/2., -n/2., -n/2.])
    sides = front+front.translate(2, n)
    darkPosTol = Formex('4:0123').replicm((n, n)).translate([-n/2., -n/2., 0.]).scale(1.-5.*tol/n).translate(2, -n/2.+1+tol)
...
def turn(x=0, y=0, action=0):
    ...
    if action==PRESS and not busy:
        ...
        C = pick(mode='element', filter='single', oneshot=True)
        print(C)
        element = C[0]
        if element == [-1]:
...
def selectElement(self, x, y, w, h):
    ...
    self.pick_parts('element', store_closest=True)
